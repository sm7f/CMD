# Inicio de Atendimento
Olá qual seria o problema ?

# Selecione a Opção certa. 
Você selecionou a opção 2 Suporte, porém a opção correta para Boletos, liberação do sistema e cobranças é a Opção 3.

# Opção 3
Você selecionou a Opção 3 de Certificado. Será necessário o arquivo do certificado para que possamos realizar a importação no computador. Você já possui o arquivo ?

# Irregularidade Fiscal
O erro que está apresentando está dizendo que você está com pendências no CNPJ, para você voltar a emitir nota será necessário entrar em contato com seu contador para verificar a sua situação com o governo.

# NFS-e Fora do ar
No momento a SEFAZ (Secretaria da Fazenda) está temporariamente indisponível para a emissão de notas de serviço. Precisamos aguardar até a normalização.

# NF-e Fora do ar
No momento a SEFAZ (Secretaria da Fazenda) está temporariamente indisponível para a emissão de notas. Precisamos aguardar até a normalização.

# Mudanças SEFAZ     
Houve uma atualização na receita onde teve mudanças na emissão de notas, vai ser necessário realizar a atualização do sistema. Vou precisar do acesso do teamviewer para inicializar a atualização. 

# Posso mandar audio
Se você puder enviar a foto do erro, será mais fácil para analisarmos e ajudarmos da melhor forma possível. Se preferir, também pode enviar um áudio explicando o problema.




# Acesso Remoto
Me informe o acesso remoto do TeamViewer

# Como posso ajudar
Para ajudar a resolver sua questão o mais rápido possível, por favor, explique detalhadamente o motivo do seu contato ou envie a foto do problema.